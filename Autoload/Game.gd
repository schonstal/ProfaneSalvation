...
func _ready():
  randomize()
  Overlay.connect("fade_complete", Callable(self, "_on_Overlay_fade_complete"))

func _on_chapter_complete():
  Overlay.fade(Color(1, 1, 1, 0.3), Color(1, 1, 1, 0), 0.5)

func reset():
  Game.change_scene_to_file("res://Scenes/Gameplay.tscn", false)

func change_scene_to_file(scene, fade_music = true):
  target_scene = scene
