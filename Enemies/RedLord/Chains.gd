...
func shoot():
  for i in range(0, chain_count):
    var y_position = i * 1080 / chain_count + rand_range(0, 25) + 100
    var x_position = i * 1320 / chain_count + rand_range(0, 25) + 500

    if i % 2 == 0:
      spawn_chain(0, y_position, PI / 2 + rand_range(-PI / 16, PI / 16))
    else:
      spawn_chain(1920, y_position, -PI / 2 - rand_range(-PI / 16, PI / 16))
